...
foreach ($stats as $server => $data) {
    ...
    if ((isset($data['time'], $data['bytes_percent'], $data['get_hits_percent'], $data['query_time'], $data['request_rate'], $data['curr_connections'],
        ...
        // Total Memory
        echo sprintf('%10s', Analysis::byteResize($data['limit_maxbytes']).'b');
        ...
        // Query Time
        echo sprintf('%5.0f', Analysis::valueResize($data['query_time'])).' ms';
        ...
        // Request rate
        echo sprintf('%8s', Analysis::valueResize($data['request_rate']));

        // Get rate
        echo sprintf('%8s', Analysis::valueResize($data['get_rate']));

        // Set rate
        echo sprintf('%8s', Analysis::valueResize($data['set_rate']));

        // Delete rate
        echo sprintf('%8s', Analysis::valueResize($data['delete_rate']));

        // Eviction rate
        if ($data['eviction_rate'] > $_ini->get('eviction_alert')) {
            echo str_pad('', 8 - strlen(Analysis::valueResize($data['eviction_rate'])), ' ').'<span class="red">'.Analysis::valueResize($data['eviction_rate']).'</span>';
        } else {
            echo sprintf('%8s', Analysis::valueResize($data['eviction_rate']));
        }

        // Bytes read
        echo sprintf('%11s', Analysis::byteResize($data['bytes_read'] / $data['time']).'b');

        // Bytes written
        echo sprintf('%10s', Analysis::byteResize($data['bytes_written'] / $data['time']).'b');
    } else {
